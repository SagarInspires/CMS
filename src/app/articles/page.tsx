    ...
    <>
      ...
      <div className="min-h-screen bg-[#050505] text-white selection:bg-white/20 selection:text-white font-sans overflow-x-hidden relative">
        ...
        <header className="fixed top-6 left-0 right-0 z-50 pointer-events-none">
          <div className="px-6 md:px-12 flex justify-between items-center w-full">
            ...
            <div className="pointer-events-auto bg-white/[0.03] backdrop-blur-3xl border border-white/[0.08] rounded-full px-2 py-1.5 flex items-center gap-2 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
              <Link href="/articles" className="px-4 py-2 text-sm font-semibold tracking-tight text-white bg-white/10 rounded-full transition-colors">
                Articles
              </Link>
        ...
        <main className="relative z-10 max-w-[90vw] mx-auto px-6 pt-48 pb-32">
          ...
          <div className="mb-24 flex flex-col md:flex-row justify-between items-end gap-12 border-b border-white/[0.08] pb-12 animate-fade-in-up">
            <div>
              <h1 className="text-[4rem] md:text-[6rem] font-sans font-bold tracking-tighter leading-[0.95] text-white">
                {q ? `Search: "${q}"` : 'The Archive'}
              </h1>
              <p className="text-xl text-white/40 font-serif italic mt-6">
            ...
            <form className="flex w-full md:w-auto gap-2" method="GET" action="/articles">
              ...
              <input 
                ...
                defaultValue={q}
                placeholder="Search..." 
                className="px-6 py-4 rounded-full bg-white/[0.03] border border-white/[0.08] text-white focus:border-white/30 focus:ring-1 focus:ring-white/30 outline-none w-full md:w-64 transition-all placeholder:text-white/20 backdrop-blur-3xl"
              />
              <button type="submit" className="w-14 h-14 bg-gradient-to-r from-white to-white/90 text-black rounded-full flex items-center justify-center hover:scale-105 transition-transform">
          ...
          {articles.length > 0 ? (
            <>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center gap-4 items-center animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                  {page > 1 && (
                    <Link href={`?${new URLSearchParams({ ...params, page: (page - 1).toString() }).toString()}`} className="px-6 py-3 rounded-full border border-white/[0.08] hover:bg-white/[0.05] hover:text-white text-white/70 transition-colors font-bold tracking-tight">
                      Previous
                    </Link>
                  )}
            ...
            <div className="py-32 text-center animate-fade-in-up">
              <h3 className="text-3xl font-serif italic text-white/30">No articles found matching your criteria.</h3>
            </div>
          )}
        </main>
      </div>
