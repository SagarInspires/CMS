...
export default async function UsersManagementPage() {
  ...
  return (
    <div className="p-12 md:p-16 h-full flex flex-col relative z-10 animate-fade-in-up">
      ...
      <div className="bg-white/[0.02] backdrop-blur-xl rounded-[2rem] border border-white/[0.05] shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-x-auto">
            ...
            {users.map(user => (
              <tr key={user.id} className="hover:bg-white/[0.04] transition-colors group">
                <td className="px-8 py-6 font-bold text-lg tracking-tight text-white/90 group-hover:text-white transition-colors">
                  {user.name}
                  {session.userId === user.id && <span className="ml-3 text-[10px] bg-white/[0.1] text-white px-3 py-1 rounded-full border border-white/20 uppercase tracking-widest font-bold">You</span>}
                </td>
                <td className="px-8 py-6 font-bold text-white/60 group-hover:text-white/80 transition-colors">{user.email}</td>
                <td className="px-8 py-6 font-serif italic text-white/40 group-hover:text-white/60 transition-colors">{new Date(user.createdAt).toLocaleDateString()}</td>
