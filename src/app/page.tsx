      ...
      <div className="min-h-screen bg-[#050505] text-white selection:bg-white/20 selection:text-white font-sans overflow-x-hidden relative">
        ...
        <header className="fixed top-6 left-0 right-0 z-50 pointer-events-none">
          <div className="px-6 md:px-12 flex justify-between items-center w-full">
            ...
            <div className="pointer-events-auto bg-white/[0.03] backdrop-blur-3xl border border-white/[0.08] rounded-full px-2 py-1.5 flex items-center gap-2 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
              ...
              {session?.isAuth ? (
                <Link href="/dashboard" className="ml-2 px-5 py-2 text-sm font-bold bg-gradient-to-r from-white to-white/90 text-black rounded-full hover:scale-[1.02] transition-transform flex items-center gap-2">
                  Dashboard <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                </Link>
              ) : (
                <Link href="/login" className="ml-2 px-5 py-2 text-sm font-bold bg-gradient-to-r from-white to-white/90 text-black rounded-full hover:scale-[1.02] transition-transform flex items-center gap-2">
                  Sign In <span className="w-2 h-2 rounded-full bg-black"></span>
                </Link>
              )}
            </div>
          </div>
        ...
        <main className="relative z-10 w-full pt-40 md:pt-48 pb-32 px-6 md:px-12">
          ...
          <section className="max-w-[90vw] mx-auto text-center mb-32 flex flex-col items-center animate-fade-in-up">
            <h2 className="text-sm font-bold tracking-tight mb-8 text-white/40 uppercase tracking-widest">editorial.flow</h2>
            <h1 className="text-[3rem] sm:text-[5rem] md:text-[6.5rem] lg:text-[8rem] font-sans font-bold tracking-tighter leading-[0.95] text-white">
              We shape <span className="font-serif italic font-normal tracking-tight text-white/80">your stories</span><br/>
            ...
            <div className="mt-24 animate-bounce opacity-50">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 4V20M12 20L6 14M12 20L18 14" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
          </section>
          ...
          {articles.length > 0 ? (
              ...
              {articles[0] && (
                <div className="md:col-span-10 flex flex-col group cursor-pointer" data-cursor="read">
                  <Link href={`/articles/${articles[0].slug}`} className="block w-full">
                    <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/[0.08] rounded-squircle aspect-square md:aspect-[4/3] w-full overflow-hidden relative mb-6 transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-[0_0_40px_rgba(255,255,255,0.05)]">
                    ...
                    <div className="flex justify-between items-center px-4">
                      <h4 className="font-bold text-xl tracking-tight text-white/90">{articles[0].title}</h4>
                      <span className="text-sm font-bold tracking-widest text-white/40 uppercase bg-white/[0.05] border border-white/10 px-4 py-1.5 rounded-full">{articles[0].category?.name || 'Uncategorized'}</span>
                    </div>
                  </Link>
                </div>
              )}
                ...
                {articles.slice(1, 3).map((article, i) => (
                  <div key={article.id} className="flex flex-col group cursor-pointer" data-cursor="read">
                    <Link href={`/articles/${article.slug}`} className="block w-full">
                      <div className="bg-white/[0.02] backdrop-blur-xl border border-white/[0.05] rounded-[2.5rem] aspect-square w-full overflow-hidden relative mb-6 transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-[0_0_30px_rgba(255,255,255,0.03)] group-hover:bg-white/[0.04]">
                        <div className="absolute inset-0 flex items-center justify-center p-8">
                           <h3 className="text-white/80 text-2xl font-serif italic text-center leading-tight group-hover:text-white transition-colors">
                             {article.title}
                           </h3>
                        </div>
                      </div>
                      <div className="flex justify-between items-center px-4">
                        <h4 className="font-bold text-lg tracking-tight text-white/80 truncate max-w-[70%]">{article.title}</h4>
                        <span className="text-xs font-bold text-white/30 tracking-wider uppercase">{article.author.name.split(' ')[0]}</span>
                      </div>
                    </Link>
                  </div>
                ))}
            ...
            <div className="py-32 text-center animate-fade-in-up">
              <h3 className="text-3xl font-serif italic text-white/30">The archive is currently empty.</h3>
            </div>
          )}
